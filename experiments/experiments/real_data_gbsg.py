...
def deepsurv_ibs(ds_model, X_train, X_test, t_train, d_train, t_test, d_test, n_time_points=50):
...
def run_gbsg_analysis():
    print("=" * 60)
    print("REAL DATA ANALYSIS: GBSG Dataset")
    print("=" * 60)
    ...
    try:
        ...
        print(f"Data loading failed: {e}"); return None
    ...
    gbsg_results = {
        "dataset": "GBSG", "n_total": len(t_obs), "n_train": len(setup["t_train"]),
    ...
    with open("gbsg_results.json", "w") as f:
        json.dump(gbsg_results, f, indent=2)
    print("\nResults saved to gbsg_results.json")
    return gbsg_results


gbsg_results = run_gbsg_analysis()
