...
def load_flchain():
    ...
    X     = df.drop(columns=["duration", "event"]).values.astype(float)
    t_obs = df["duration"].values.astype(float)
    delta = df["event"].values.astype(float)
    return t_obs, delta, X


def run_one_split(t_obs, delta, X, seed, hparams, ds_hparams,
                  rsf_kwargs=None):
    ...
    cox_ibs = cox_integrated_brier_score(cox_model, setup)
    ...
    ds_ibs = deepsurv_ibs(ds_model, setup["X_train"], setup["X_test"],
    ...
    rsf_ibs = rsf_integrated_brier_score(
        rsf_model, setup["X_train"], setup["X_test"],
        setup["t_train"], setup["d_train"], t_test, d_test)

    return dict(seed=seed, best_epoch=best_epoch, h_viol=h_viol,
                mnshe_c=mnshe_c, mnshe_ibs=mnshe_ibs,
                cox_c=cox_c, cox_ibs=cox_ibs,
                ds_c=ds_c, ds_ibs=ds_ibs,
                rsf_c=rsf_c, rsf_ibs=rsf_ibs)


def run_repeated_splits(name, loader, seeds, out_csv, hparams, ds_hparams,
                        rsf_kwargs=None):
    t_obs, delta, X = loader()
    print(f"{name}: n={len(t_obs)}  p={X.shape[1]}  event rate={delta.mean():.3f}")

    done_seeds = set()
    if os.path.exists(out_csv):
        done_seeds = set(pd.read_csv(out_csv)["seed"].tolist())

    for seed in seeds:
        if seed in done_seeds:
            continue
        r = run_one_split(t_obs, delta, X, seed, hparams, ds_hparams,
        ...
        print(f"  seed={seed:4d}  H_viol={r['h_viol']}  |  "
              f"MNSHE C={r['mnshe_c']:.4f} IBS={r['mnshe_ibs']:.4f}  |  "
              f"Cox C={r['cox_c']:.4f} IBS={r['cox_ibs']:.4f}  |  "
              f"DS C={r['ds_c']:.4f} IBS={r['ds_ibs']:.4f}  |  "
              f"RSF C={r['rsf_c']:.4f} IBS={r['rsf_ibs']:.4f}")

    return pd.read_csv(out_csv)


def summarise_splits(df, name):
    print(f"\n{name}  --  {len(df)} splits  --  mean (SD)")
    for col_c, col_i, lbl in [
        ("mnshe_c", "mnshe_ibs", "MNSHE"), ("cox_c", "cox_ibs", "Cox  "),
        ("ds_c", "ds_ibs", "DS   "), ("rsf_c", "rsf_ibs", "RSF  "),
    ]:
        print(f"  {lbl}  C={df[col_c].mean():.4f} ({df[col_c].std():.4f})   "
              f"IBS={df[col_i].mean():.4f} ({df[col_i].std():.4f})")
    print(f"  H violations, total: {df['h_viol'].sum()}")


if __name__ == "__main__":
    hparams    = json.load(open("best_hparams.json"))
    ds_hparams = json.load(open("best_ds_hparams.json"))
    seeds = list(range(0, 2000, 100))

    df = run_repeated_splits(
        "FLCHAIN", load_flchain, seeds, "../results/flchain_splits.csv",
        hparams, ds_hparams,
        # rsf_kwargs={"min_samples_leaf": 10},  # uncomment if RSF runs out of memory
    )
    summarise_splits(df, "FLCHAIN")
